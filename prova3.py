...
CONFIDENCE_THRESHOLD = 75 # porcentaje mínimo para considerar una letra válida
STABILITY_FRAMES = 4 #número de frames consecutivos para confirmar un cambio de letra
COOLDOWN_FRAMES = 2 #número de frames para esperar antes de considerar un nuevo cambio
K_NEIGHBORS = 10 #número de vecinos más cercanos para la clasificación
TEMPORAL_SMOOTHING = 3 #suavizado temporal para reducir ruido en las landmarks

# ==========================
# NUEVOS PESOS DE FEATURES
# ==========================
FEATURE_WEIGHTS = np.array( #lo de dentro los corchetes son los pesos
    [2.5]*9 +      # flexiones dedos
    [4]*5 +    # distancias
    [2.5]*5 +    # extensiones
    [2]*5 +      # ángulos entre dedos
    [1]*3 +      # orientación
    [0.5]*3      # brazo
)

# FUNCIONES AUXILIARES
def angulo(a,b,c): #ángulo entre tres puntos (en b)
    ba = a-b
    bc = c-b
...
def angulo_vectores(v1, v2): #angulo entre dos vectores desde la muñeca (v1 y v2 son vectores desde la muñeca a los dedos)
    cosang = np.dot(v1, v2)/(np.linalg.norm(v1)*np.linalg.norm(v2)+1e-6)
    cosang = np.clip(cosang,-1,1)
    return np.arccos(cosang)


def extraer_features(vector): #extrae características relevantes de un vector de landmarks (brazo + mano)
    ...
    features = np.concatenate([
        ...
        distancias,
        extensiones,
        angulos_entre_dedos,   # nuevas features
        orientacion,
        brazo_features
    ])
...
class Comparador:
    ...
    def comparar(self,vector):
        ...
        features = extraer_features(vector) #extraemos las features del vector de entrada
        features = (features-self.mean)/self.std #normalizamos con la media y desviación del dataset

        # aplicar pesos
        diff = (self.features - features) * FEATURE_WEIGHTS

        dists = np.linalg.norm(diff,axis=1) #calculamos la distancia euclidiana ponderada a cada muestra del dataset

        idx = np.argsort(dists)[:self.k] #obtenemos los índices de las k muestras más cercanas

        vecinos = self.labels[idx] #obtenemos las letras correspondientes a esas muestras
        distancias = dists[idx] #obtenemos las distancias a esas muestras

        votos = {}

        for letra,dist in zip(vecinos,distancias):

            peso = 1/(dist+1e-6) #peso inversamente proporcional a la distancia (más cerca = más peso)

            votos[letra] = votos.get(letra,0) + peso #sumamos los pesos para cada letra
